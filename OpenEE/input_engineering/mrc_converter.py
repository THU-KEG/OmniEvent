...
def make_preditions(all_start_logits, all_end_logits, training_args):
        ...
        for argument in arguments["arguments"]:
            for mention in argument["mentions"]:
                arguments_per_trigger.append(
                    (event_argument_type, (mention["position"][0], mention["position"][1]), example_id))
    ...
    for example_id, (start_logits, end_logits) in enumerate(zip(all_start_logits, all_end_logits)):
        event_argument_type = data_for_evaluation["golden_arguments"][example_id]["pred_type"] + "_" + \
                              data_for_evaluation["golden_arguments"][example_id]["role"]
