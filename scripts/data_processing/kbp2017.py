"""
@ File:    tac-kbp2017_sent.py
@ Author:  Zimu Wang
# Update:  June 10, 2022
@ Purpose: Convert the TAC KBP 2017 dataset in sentence level.
"""
import copy
import re
import jsonlines
...
class Config(object):
    ...
    def __init__(self):
        ...
        self.EVAL_DATA_FOLDER = os.path.join(self.PROJECT_FOLDER, 'tac_kbp_event_arg_comp_train_eval_2016-2017/'
                                                                  'data/2017/eval/eng')
        ...
        self.EVAL_SOURCE_FOLDER_DF = os.path.join(self.PROJECT_FOLDER, 'tac_kbp_eval_src_2016-2017/data/2017/'
                                                                       'eng/df')
        self.EVAL_SOURCE_FOLDER_NW = os.path.join(self.PROJECT_FOLDER, 'tac_kbp_eval_src_2016-2017/data/2017/'
                                                                       'eng/nw')

        # The configurations for the saving path.
        self.SAVE_DATA_FOLDER = os.path.join(self.PROJECT_FOLDER, 'tac_kbp_event_arg_comp_train_eval_2016-2017/'
                                                                  'TAC-KBP2017')
        ...
        for event in document['events']:
            for trigger in event['triggers']:
                if not document['text'][trigger['position'][0]:trigger['position'][1]] \
                    ...
                    trigger['trigger_word'] = re.sub('&amp;', '&', trigger['trigger_word'])
                    if document['id'] == 'ENG_DF_001503_20090218_G00A0GAXK' \
                            and trigger['trigger_word'] == 'a dullard on here ( Noel O\'Gara) whowrote a book aout it':
                        trigger['trigger_word'] = 'a dullard on here ( Noel O\'Gara) who wrote a book aout it'
                for argument in trigger['arguments']:
                    for mention in argument['mentions']:
                        if document['text'][mention['position'][0]:mention['position'][1]] \
                                != mention['mention']:
                            mention['mention'] = re.sub('<.*?>', '', mention['mention'].strip())
                            mention['mention'] = re.sub('  ', ' ', mention['mention'])
                            mention['mention'] = re.sub('amp;', '', mention['mention'])
                            if document['id'] == 'ENG_DF_001503_20090218_G00A0GAXK' \
                                    and mention['mention'] == \
                                    'a dullard on here ( Noel O\'Gara) whowrote a book aout it':
                                mention['mention'] = 'a dullard on here ( Noel O\'Gara) who wrote a book aout it'
            for entity in document['entities']:
                for mention in entity['mentions']:
                    if not document['text'][mention['position'][0]:mention['position'][1]] \
                        ...
                        mention['mention'] = re.sub('amp;', '', mention['mention'])
                        if document['id'] == 'ENG_DF_001503_20090218_G00A0GAXK' \
                                and mention['mention'] == 'a dullard on here ( Noel O\'Gara) whowrote a book aout it':
                            mention['mention'] = 'a dullard on here ( Noel O\'Gara) who wrote a book aout it'
